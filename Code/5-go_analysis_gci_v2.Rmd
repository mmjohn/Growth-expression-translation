...
```{r}
data_pos_high <- read_csv(here('Data', 'genes_GCI', 'ecoli_full_pos_corr_high_exp.csv'))
data_pos_low <- read_csv(here('Data', 'genes_GCI', 'ecoli_full_pos_corr_low_exp.csv'))
data_neg_high <- read_csv(here('Data', 'genes_GCI', 'ecoli_full_neg_corr_high_exp.csv'))
data_neg_low <- read_csv(here('Data', 'genes_GCI', 'ecoli_full_neg_corr_low_exp.csv'))
...
```{r}
data_pos_high$ENTREZID <- mapIds(org.EcK12.eg.db,
                              keys = data_pos_high$gene_name,
                              column = "ENTREZID",
                              keytype = "SYMBOL",
                              multiVals = "first")

data_pos_low$ENTREZID <- mapIds(org.EcK12.eg.db,
                              keys = data_pos_low$gene_name,
                              column = "ENTREZID",
                              keytype = "SYMBOL",
                              multiVals = "first")

data_neg_high$ENTREZID <- mapIds(org.EcK12.eg.db,
                              keys = data_neg_high$gene_name,
                              column = "ENTREZID",
                              keytype = "SYMBOL",
                              multiVals = "first")

data_neg_low$ENTREZID <- mapIds(org.EcK12.eg.db,
                              keys = data_neg_low$gene_name,
                              column = "ENTREZID",
                              keytype = "SYMBOL",
                              multiVals = "first")
...
```{r}
data_pos_high <- dplyr::distinct(data_pos_high, gene_name, .keep_all= TRUE)
data_pos_low <- dplyr::distinct(data_pos_low, gene_name, .keep_all= TRUE)
data_neg_high <- dplyr::distinct(data_neg_high, gene_name, .keep_all= TRUE)
data_neg_low <- dplyr::distinct(data_neg_low, gene_name, .keep_all= TRUE)
```
...
```{r}
# removing genes without entrezID
mtx_pos_high <- subset(data_pos_high, is.na(ENTREZID) == FALSE)
mtx_pos_low <- subset(data_pos_low, is.na(ENTREZID) == FALSE)
mtx_neg_high <- subset(data_neg_high, is.na(ENTREZID) == FALSE)
mtx_neg_low <- subset(data_neg_low, is.na(ENTREZID) == FALSE)

# creating a new matrix
gene_matrix_pos_high <- mtx_pos_high$gci
gene_matrix_pos_low <- mtx_pos_low$gci
gene_matrix_neg_high <- mtx_neg_high$gci
gene_matrix_neg_low <- mtx_neg_low$gci

# add entrezID as rownames 
names(gene_matrix_pos_high) <- mtx_pos_high$ENTREZID
names(gene_matrix_pos_low) <- mtx_pos_low$ENTREZID
names(gene_matrix_neg_high) <- mtx_neg_high$ENTREZID
names(gene_matrix_neg_low) <- mtx_neg_low$ENTREZID

gene_list_pos_high <- gene_matrix_pos_high[abs(gene_matrix_pos_high) > 0]
gene_list_pos_low <- gene_matrix_pos_low[abs(gene_matrix_pos_low) > 0]
gene_list_neg_high <- gene_matrix_neg_high[abs(gene_matrix_neg_high) > 0]
gene_list_neg_low <- gene_matrix_neg_low[abs(gene_matrix_neg_low) > 0]
...
```{r}
go_enrich_pos_high <- enrichGO(
  gene=names(gene_list_pos_high),
  OrgDb= 'org.EcK12.eg.db',
  keyType = "ENTREZID",
  ont = "ALL",
  pvalueCutoff = 0.04,
...
summary_go_pos_high <- as.data.frame(go_enrich_pos_high)

go_enrich_pos_low <- enrichGO(
  gene=names(gene_list_pos_low),
  OrgDb= 'org.EcK12.eg.db',
  keyType = "ENTREZID",
  ont = "ALL",
  pvalueCutoff = 0.04,
...
summary_go_pos_low <- as.data.frame(go_enrich_pos_low)

go_enrich_neg_high <- enrichGO(
  gene=names(gene_list_neg_high),
  OrgDb= 'org.EcK12.eg.db',
  keyType = "ENTREZID",
  ont = "ALL",
  pvalueCutoff = 0.04,
...
summary_go_neg_high <- as.data.frame(go_enrich_neg_high)

go_enrich_neg_low <- enrichGO(
  gene=names(gene_list_neg_low),
  OrgDb= 'org.EcK12.eg.db',
  keyType = "ENTREZID",
  ont = "ALL",
  pvalueCutoff = 0.04,
...
summary_go_neg_low <- as.data.frame(go_enrich_neg_low)
```
...
```{r}
df_go_pos_high <- summary_go_pos_high %>% 
  select(c(Description, GeneRatio, BgRatio, p.adjust)) %>% # keep only necessary columns
  extract( # split ratios into numerator and denominator to build contingency table
    GeneRatio, 
    into = c("gene_num", "gene_denom"), 
...
df_go_pos_low <- summary_go_pos_low %>% 
  select(c(Description, GeneRatio, BgRatio, p.adjust)) %>% # keep only necessary columns
  extract( # split ratios into numerator and denominator to build contingency table
    GeneRatio, 
    into = c("gene_num", "gene_denom"), 
...
df_go_neg_high <- summary_go_neg_high %>% 
  select(c(Description, GeneRatio, BgRatio, p.adjust)) %>% # keep only necessary columns
  extract( # split ratios into numerator and denominator to build contingency table
    GeneRatio, 
    into = c("gene_num", "gene_denom"), 
...
df_go_neg_low <- summary_go_neg_low %>% 
  select(c(Description, GeneRatio, BgRatio, p.adjust)) %>% # keep only necessary columns
  extract( # split ratios into numerator and denominator to build contingency table
    GeneRatio, 
    into = c("gene_num", "gene_denom"), 
...
```{r}
fig_go_pos_high <- df_go_pos_high %>% 
  arrange(desc(odds_ratio)) %>% 
  slice(1:10) %>% 
  mutate(labels = str_wrap(Description, 30)) %>% 
  ggplot() +
  ...
  geom_vline(xintercept = 1) +
  #scale_x_continuous(name = "Odds Ratio") +
  scale_x_log10(name = "Odds Ratio", limits = c(1, 100)) +
  scale_y_discrete(name = NULL) +
  ggtitle("GCI > 0, high avg. expr.") +
  theme_bw()
```

```{r}
fig_go_pos_low <- df_go_pos_low %>% 
  arrange(desc(odds_ratio)) %>% 
  slice(1:10) %>% 
  mutate(labels = str_wrap(Description, 30)) %>% 
  ggplot() +
  ...
  scale_x_log10(name = "Odds Ratio", limits = c(1, 100)) +
  scale_y_discrete(name = NULL) +
  ggtitle("GCI > 0, low avg. expr.") +
  theme_bw()
```

```{r}
fig_go_neg_high <- df_go_neg_high %>% 
  arrange(desc(odds_ratio)) %>% 
  slice(1:10) %>% 
  mutate(labels = str_wrap(Description, 30)) %>% 
  ggplot() +
...
```{r}
full_fig <- (fig_go_pos_low + fig_go_pos_high) / (fig_go_neg_low + fig_go_neg_high)

save_plot(
  here('Manuscript', 'Figures', 'fig_go_by_gci_exp.png'),
  full_fig, 
  ncol = 1, 
  nrow = 1, 
