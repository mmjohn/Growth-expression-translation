...
# quick look at GCI distributions for AT-rich genes vs non-AT-rich genes
```{r}
...
at_rich_test_df <- 
  full_join(data, neg_data) %>% 
  mutate(
    set = case_when(
      locus_tag %in% at_rich_df$gene ~ 'AT-rich',
      !locus_tag %in% at_rich_df$gene ~ 'not AT-rich'
    )
  )

t_test_at_results <- t.test(filter(at_rich_test_df, set=='AT-rich')$GCI, filter(at_rich_test_df, set=='not AT-rich')$GCI)
t_test_at_results$p.value  # 3.276559e-73
...
fig_at_gci <- at_rich_test_df %>%  
  ggplot(aes(x = GCI, fill = set)) +
  geom_density(alpha = 0.5) +
  geom_richtext(
    data = ttest_text_at,
    aes(x = x, y = y, label = label),
