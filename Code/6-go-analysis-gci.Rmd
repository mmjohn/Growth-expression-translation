...
data <- read_csv(glue::glue('{path_to_data}ecoli_full_pos_corr.csv'))
neg_data <- read_csv(glue::glue('{path_to_data}ecoli_full_neg_corr.csv'))
...
## Add in core vs accessory analysis
```{r}

path_to_data <- '/Users/mackenziejohnson/Documents/grad_school/wilke_lab/Growth-expression-translation/Data/'

meta_df <- read_tsv(glue::glue({path_to_data}, 'processed_data/', 'processed_metadata_ecoli.tsv'))

exp_df <- read_tsv(glue::glue({path_to_data}, 'processed_data/', 'processed_expression_ecoli.tsv'))

full_df <- read_tsv(glue::glue({path_to_data}, 'ecoli_info/', 'current_ecoli_master_table.tsv')) %>% 
  select(locus_tag, gene)

core_df <- read_tsv(
  file = glue::glue({path_to_data}, 'ecoli_info/', 'essential_ecoli_genes.txt')
) %>% 
...
full_df <- full_df %>% 
  mutate(
    genome = case_when(
      gene %in% core_df$`Gene Name` ~ "core",
      TRUE ~ "accessory"
    )) 
...
core_gci <- gci_genomes_df %>% 
  filter(genome == "core") %>% 
  select(pearsons)

access_gci <- gci_genomes_df %>% 
  filter(genome == "accessory") %>% 
  select(pearsons)

t_test_results <- t.test(core_gci, access_gci)
...
fig_core_gci <- gci_genomes_df %>% 
  drop_na(genome) %>% 
  ggplot(aes(x = pearsons, fill = genome)) +
...
save_plot(
  file.path(
    path_to_figures,  
    glue::glue('fig_core_go_gci.png')   # file name
  ),
  fig_6, 
  ncol = 1, 
  nrow = 1, 
