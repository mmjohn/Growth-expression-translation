...
label_inf <- tibble(
  x = 12.5,
  y = str_wrap("regulation of bacterial-type flagellum-dependent cell motility", 30),
  lab = "Inf.",
  infinite = TRUE
)
...
fig_go_neg <- df_go_neg_gci %>% 
  ...
  mutate(
    ...
    infinite = case_when(odds_ratio != Inf ~ FALSE, odds_ratio == Inf ~ TRUE)
  ) %>% 
  ggplot() +
  geom_point(
    aes(
  ...
  scale_x_continuous(name = "Odds Ratio", oob = scales::squish_infinite) +
  scale_y_discrete(name = NULL) +
  scale_color_manual(
...
meta_df <- read_tsv(glue({path_to_data}, 'processed_data/', 'processed_metadata_ecoli.tsv'))

exp_df <- read_tsv(glue({path_to_data}, 'processed_data/', 'processed_expression_ecoli.tsv'))

full_df <- read_tsv(glue({path_to_data}, 'ecoli_info/', 'current_ecoli_master_table.tsv')) %>% 
  select(locus_tag, gene)

core_df <- read_tsv(
  file = glue({path_to_data}, 'ecoli_info/', 'essential_ecoli_genes.txt')
) %>% 
