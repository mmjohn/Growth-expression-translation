...
```{r}
mean_avg_exp <- mean(exp_sum_df$mean)

genes_pos_high <- exp_sum_df %>% 
  filter(lin_r > 0, mean > mean_avg_exp) %>% 
  select(`log-TPM`, mean, lin_r) %>% 
    left_join(., full_df, by = c("log-TPM" = "locus_tag"))

genes_pos_low <- exp_sum_df %>% 
  filter(lin_r > 0, mean <= mean_avg_exp) %>% 
  select(`log-TPM`, mean, lin_r) %>% 
    left_join(., full_df, by = c("log-TPM" = "locus_tag"))

genes_neg_high <- exp_sum_df %>% 
  filter(lin_r < 0, mean > mean_avg_exp) %>% 
  select(`log-TPM`, mean, lin_r) %>% 
    left_join(., full_df, by = c("log-TPM" = "locus_tag"))

genes_neg_low <- exp_sum_df %>% 
  filter(lin_r < 0, mean <= mean_avg_exp) %>% 
  select(`log-TPM`, mean, lin_r) %>% 
    left_join(., full_df, by = c("log-TPM" = "locus_tag"))

colnames(genes_pos_high) <- c("locus_tag", "mean_exp", "gci", "gene_name")
colnames(genes_pos_low) <-c("locus_tag", "mean_exp", "gci", "gene_name")
colnames(genes_neg_high) <- c("locus_tag", "mean_exp", "gci", "gene_name")
colnames(genes_neg_low) <- c("locus_tag", "mean_exp", "gci", "gene_name")
...
write_csv(
  ...
  here(
    'Data',
    'genes_GCI',
    glue('{data_tag}_pos_corr_high_exp.csv')
  ),
  append = FALSE
)
...
write_csv(
  ...
  here(
    'Data',
    'genes_GCI',
    glue('{data_tag}_pos_corr_low_exp.csv')
  ),
  append = FALSE
)
...
write_csv(
  ...
  here(
    'Data',
    'genes_GCI',
    glue('{data_tag}_neg_corr_high_exp.csv')
  ),
  append = FALSE
)
...
write_csv(
  ...
  here(
    'Data',
    'genes_GCI',
    glue('{data_tag}_neg_corr_low_exp.csv')
  ),
  append = FALSE
)
